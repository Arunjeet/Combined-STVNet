...
def trainModel(modelStruct, modelGen, modelClass = 'cat', batchSize = 1, optimizer = tf.keras.optimizers.Adam, learning_rate = 0.01, losses = None, metrics = ['accuracy'], saveModel = True, modelName = 'stvNet_weights', epochs = 1, loss_weights = None, outVectors = False, outClasses = False, dataSplit = True, altLabels = True, augmentation = True): # train and save model weights
	...
	if type(losses) is dict:
		...
		if len(outKeys) == 2: # combined output
			for i in range(epochs):
				...
				hist = model.fit(modelGen(modelClass, batchSize, masterList = trainData, out0 = outKeys[0], out1 = outKeys[1], altLabels = False, augmentation = augmentation), steps_per_epoch = math.ceil(len(trainData) / batchSize), max_queue_size = 2, callbacks = [logger])
...
def trainModels(modelSets, shutDown = False):
	for modelSet in modelSets:
		...
		trainModel(model.structure, model.generator, modelClass = modelSet.modelClass, epochs = model.epochs, losses = model.losses, modelName = modelSet.name, outClasses = model.outClasses, outVectors = model.outVectors, learning_rate = model.lr, metrics = model.metrics, altLabels = False, augmentation = model.augmentation)
...
def evaluateModel(modelStruct, modelName, evalGen, modelClass = 'cat', outVectors = False, outClasses = False, batchSize = 1, optimizer = tf.keras.optimizers.Adam, learning_rate = 0.01, losses = None, metrics = ['accuracy'], samples = 100): # test existing model performance
...
def evaluateModels(modelSets, batchSize = 1, dataSplit = True):
...
def trainModelClassGen(modelStruct, modelName, losses, modelClass = 'cat', batchSize = 1, optimizer = tf.keras.optimizers.Adam, learningRate = 0.001, metrics = ['accuracy'], epochs = 1, outVectors = False, outClasses = False, outVecName = None, outClassName = None): # simulates generator behaviour, unused
	model = modelStruct(outVectors = outVectors, outClasses = outClasses, modelName = modelName)
	#model.summary()
...
class generatorClass: # simulates generator behaviour, unused
	...
	def __init__(self, modelClass, height = 480, width = 640, batchSize = 1, outVectors = False, outClasses = False, outVecName = None, outClassName = None):
...
modelsDict = {
	'uNet_classes' : modelDictVal(uNet, data.classTrainingGenerator, tf.keras.losses.BinaryCrossentropy(), False, True, epochs = 5, lr = 0.001,altLabels=False, augmentation = False),
	...
	'stvNet_new_classes' : modelDictVal(stvNetNew, data.classTrainingGenerator, tf.keras.losses.BinaryCrossentropy(), False, True, epochs = 5, lr = 0.001,altLabels=False, augmentation = False),
	'stvNet_new_combined' : modelDictVal(stvNetNew, data.combinedTrainingGenerator, {'coordsOut': tf.keras.losses.Huber(), 'classOut': tf.keras.losses.BinaryCrossentropy()}, True, True, epochs = 1, lr = 0.001, metrics = {'coordsOut': ['mae', 'mse'], "classOut": ['accuracy']}, altLabels=False, augmentation = False),
}
...
if __name__ == "__main__" :
	...
	#modelSets = [modelSet('stvNet_new_classes'), modelSet('uNet_classes')] # class outputs
